  ...
  try {
    res.status(StatusCodes.OK).json({
      ...
      result: {
        ...
        email: req.user.email,
        role: req.user.role,
        cart: req.user.cartQuantity // 因為models\users.js 使用了 schema.virtual('cartQuantity').get() 所以能用更簡潔的寫法
        // cart: req.user.cart.reduce((total, current) => { // 計算購物車的總數*
        //   return total + current.quantity
        // }, 0)
      }
    })
